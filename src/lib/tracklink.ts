...
import { WEB_ORIGIN } from './listen'

// Discord rejects a button URL over 512 characters, and the whole activity with
// it. Headroom below that, since WEB_ORIGIN is overridable and may be longer.
const URL_LIMIT = 450
const MAX_TITLE = 70
const MAX_ARTIST = 45
...
export function trackUrl(t: Track): string {
  const base = `${WEB_ORIGIN}/play/${t.source}/${encodeURIComponent(t.id)}`
