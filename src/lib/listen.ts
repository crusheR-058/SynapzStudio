...
/** Where a shared room link points. Overridable for previews/self-hosting. */
export const WEB_ORIGIN =
  ((import.meta as any).env?.VITE_WEB_ORIGIN as string) || 'https://synapz-music.vercel.app'

export const roomUrl = (code: string) => `${WEB_ORIGIN}/listen/${code}`
...
export async function hostRoom(hostName: string): Promise<HostSession | null> {
  ...
  const { error } = await supabase.from('listen_rooms').insert({
    code,
    host_id: user.id,
    host_name: hostName,
    position_sec: 0,
  ...
  const channel = supabase.channel(`listen:${code}`, {
  ...
  const close = async () => {
    ...
    try {
      await supabase!.from('listen_rooms').delete().eq('code', code)
    } catch {
...
export async function joinRoom(
  ...
  const apply = (state: TickPayload) => {
    ...
    if (controls.getTrackId() !== state.track.id) {
      ...
      if (loadSeek) window.clearTimeout(loadSeek)
      loadSeek = window.setTimeout(() => {
        loadSeek = null
