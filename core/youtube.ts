 ...
 *
 * Two paths, chosen by whether a Data API key is available:
 *   - Web / desktop: call Google directly with a key, stored via core/storage
 *     (localStorage on those platforms) or supplied through config.
 *   - Mobile: no key, ever — the project key is HTTP-referrer-restricted and a
 *     native app has no referrer to send. It goes through the hosted
 *     /yt/search proxy, where the key lives server-side and the CDN plus a
 *     6-hour server cache make repeat queries free.
 *
 ...
 *    (3) CACHE every result by query via core/storage, so a repeated search
...
export function getYtKey(): string {
  try {
    return storage().get(KEY_LS) || env().youtubeKey
  } catch {
    return ''
  }
}
...
// The Data API returns titles with HTML entities. Decoded by hand rather than
// through a <textarea>, which does not exist under React Native — and which
// would also happily interpret markup in a title.
...
const CACHE_MAX = 120 // cap stored queries so the store can't balloon
...
function writeCache(key: string, tracks: Track[]): void {
  try {
    storage().set(CACHE_PREFIX + key, JSON.stringify({ t: Date.now(), tracks }))
    let idx: { k: string; t: number }[] = []
    try {
      idx = JSON.parse(storage().get(CACHE_INDEX) || '[]')
    } catch {
      idx = []
    }
    ...
    while (idx.length > CACHE_MAX) {
      const old = idx.shift()
      if (old) storage().remove(CACHE_PREFIX + old.k)
    }
    storage().set(CACHE_INDEX, JSON.stringify(idx))
  } catch {
...
async function runSearch(query: string, opts: SearchOpts): Promise<Track[]> {
  const hasProxy = env().hasSearchProxy

  // Prefer a direct Data-API call whenever a key is available. On mobile there
  // never is one: the project key is referrer-restricted and a native app has no
  // referrer to send, so mobile always takes the proxy branch below.
  if (hasYtKey()) {
    try {
      return await searchYouTube(query, 50, opts)
    } catch (e) {
      if (!hasProxy) throw e // nothing to fall back to — surface the error
      // else fall through to the hosted /yt/search proxy
    }
  }
  if (!hasProxy) throw new YtError('NO_KEY')
