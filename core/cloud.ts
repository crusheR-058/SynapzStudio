...
export async function cloudRemoveLike(trackId: string): Promise<void> {
  ...
  if (!sb() || !id) return
  try {
    await sb()!.from('liked_tracks').delete().eq('user_id', id).eq('track_id', trackId)
  } catch {
    /* best-effort */
  }
}
...
export async function cloudImportLikes(localLikes: Track[]): Promise<void> {
  const id = await uid()
  if (!sb() || !id || !localLikes.length) return
  try {
    const rows = localLikes.map((t) => ({ user_id: id, track_id: t.id, track: t }))
    await sb()!.from('liked_tracks').upsert(rows, { onConflict: 'user_id,track_id' })
  } catch {
    /* best-effort */
  }
}
...
export async function cloudRecordPlay(track: Track): Promise<void> {
  ...
  if (!sb() || !id) return
  try {
    await sb()!.from('play_history').insert({ user_id: id, track_id: track.id, track })
  } catch {
    /* best-effort */
  }
}
...
export async function cloudDeletePlaylist(playlistId: string): Promise<void> {
  if (!sb()) return
  try {
    await sb()!.from('playlists').delete().eq('id', playlistId)
  } catch {
    /* best-effort */
  }
}
...
export async function cloudAddToPlaylist(playlistId: string, track: Track): Promise<void> {
  ...
  try {
    ...
    await sb()!.from('playlists').update({ updated_at: new Date().toISOString() }).eq('id', playlistId)
  } catch {
    /* best-effort */
  }
}
